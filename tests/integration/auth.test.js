...
describe('Auth routes', () => {
	...
	describe('POST /v1/auth/verify-account', () => {
		test('should return 204 and verify user', async () => {
			...
			const verificationToken = tokenService.generateToken(userOne._id, expires, tokenTypes.ACCOUNT_VERIFICATION);
			await tokenService.saveToken(verificationToken, userOne._id, expires, tokenTypes.ACCOUNT_VERIFICATION);
			...
			const dbVerificationTokenCount = await Token.countDocuments({
				user: userOne._id,
				type: tokenTypes.ACCOUNT_VERIFICATION,
			});
		...
		test('should return 401 if verification token is blacklisted', async () => {
			...
			const expires = moment().add(config.jwt.verificationExpirationDays, 'days');
			const verificationToken = tokenService.generateToken(userOne._id, expires, tokenTypes.ACCOUNT_VERIFICATION);
			await tokenService.saveToken(verificationToken, userOne._id, expires, tokenTypes.ACCOUNT_VERIFICATION, true);
...
describe('Auth middleware', () => {
	...
	test('should call next with unauthorized error if access token is expired', async () => {
		...
		const accessToken = tokenService.generateToken(userOne._id, tokenExpires, tokenTypes.ACCESS);
		const req = httpMocks.createRequest({ headers: { Authorization: `Bearer ${accessToken}` } });
		const next = jest.fn();
